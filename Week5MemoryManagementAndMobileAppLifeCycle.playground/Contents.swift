...
//: - Total cost and balance should be a read-only computed variables. The logic for total cost should use the reduce higher order function on the cart. If an item has no cost stored in the optional value, return the accumulating total. Otherwise, return the cost of the item multiplied times it's quantity added to the accumulating total. The logic for balance should simply subtract the total from the budget.
...
//: - Write an GroceryTripError enum that conforms to the Error protocol for the following cases: If the total exceeds the budget. If an item is selected that is not on the shopping list. If the quantity exceeds the required amount. If the quantity falls short of the required amount.
...
//: - Write a function to checkout that can throw an error. This function will return the remaining items on the shopping list and the remaining budget in a tuple. If the balance is negative, throw the appropriate error. Otherwise, remove everything from the shopping list whose boolean evaluates to true and return everything on the shopping list that wasn't purchased, and return the remaining available budget amount. Do not return a dictionary, but return an array of GroceryItem.
